...
SpasEnum::SpasEnum(string word) : Sequence(word) {
	reset(word);
}
...
string SpasEnum::mod() {
	...
	if (isActive()) { //Concatenate if "active" or true
		...
		classModWord = temp.append(subsequence);
	}
	else { // truncate
		cout << "The word was truncated." << endl;
		if (temp.find(subsequence) != std::string::npos) {
			string::size_type i = temp.find(subsequence);
			while (i != string::npos) {
				classModWord = temp.erase(i, subsequence.length());
				i = temp.find(subsequence, i);
			}
		}
	}
	state = false;
